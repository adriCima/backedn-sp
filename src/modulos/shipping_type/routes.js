...
async function del(req, res, next){
    try {
        ...
        respuesta.success(req, res, 'Tipo de entrega eliminado correctamente' , 200)
    } catch (err) {
        next(err);
    }
};
...
async function insert(req, res, next){
    try {
        ...
        if(req.body.id == 0){
            mensaje = 'Tipo de entrega guardado con éxito'
        }else{
            mensaje = 'Tipo de entrega actualizado con éxito'
        }
