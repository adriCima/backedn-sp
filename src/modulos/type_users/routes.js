...
async function del(req, res, next){
    try {
        ...
        respuesta.success(req, res, 'Tipo de usuario eliminado correctamente' , 200)
    } catch (err) {
        next(err);
    }
};
...
async function insert(req, res, next){
    try {
        ...
        if(req.body.id == 0){
            mensaje = 'Tipo de usuario guardado con éxito'
        }else{
            mensaje = 'Tipo de usuario actualizado con éxito'
        }
