...
async function del(req, res, next){
    try {
        ...
        respuesta.success(req, res, 'status de pago eliminado correctamente' , 200)
    } catch (err) {
        next(err);
    }
};
...
async function insert(req, res, next){
    try {
        ...
        if(req.body.id == 0){
            mensaje = 'status de pago guardado con éxito'
        }else{
            mensaje = 'status de pago actualizado con éxito'
        }
